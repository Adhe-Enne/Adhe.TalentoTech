                ...
                <div className="px-6 pb-6">
                  ...
                  {position && (
                    <div className="flex justify-center mt-2">
                      <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-brand/10 text-brand">{position}</span>
                    </div>
                  )}
                  {bio && <p className="text-sm text-gray-500 leading-relaxed mt-3 text-center line-clamp-3">{bio}</p>}
                  <div className="border-t border-gray-100 mt-4 pt-4 flex justify-center flex-wrap gap-2">
                    {bio && (
                      <button
                        aria-label={`Ver más bio de ${name}`}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-gray-200 text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-all duration-200"
                        onClick={() => onShowMore(p)}
                        type="button"
                      >
                        Ver más
                      </button>
                    ...
                    {email && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-brand/20 text-brand hover:bg-brand/5 hover:border-brand/40 transition-all duration-200"
                        href={`mailto:${email}`}
                      >
                    ...
                    {linkedin && (
                      <a
                        ...
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium border border-accent/20 text-accent hover:bg-accent/5 hover:border-accent/40 transition-all duration-200"
                        href={linkedin}
              ...
              <div className="px-8 pb-8">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    ...
                    {selected.position && <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-brand/10 text-brand mt-2">{selected.position}</span>}
                  </div>
                ...
                <div className="mt-4 text-sm leading-relaxed text-gray-600">{selected.bio ? <p>{selected.bio}</p> : <p className="text-gray-400 italic">Sin biografía disponible.</p>}</div>
                <div className="border-t border-gray-100 mt-6 pt-4 flex flex-wrap gap-2">
                  {selected.email && (
                    <a
                      aria-label={`Email de ${selected.name}`}
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-brand/20 text-brand hover:bg-brand/5 hover:border-brand/40 transition-all duration-200"
                  ...
                  {selected.linkedin && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium border border-accent/20 text-accent hover:bg-accent/5 hover:border-accent/40 transition-all duration-200"
